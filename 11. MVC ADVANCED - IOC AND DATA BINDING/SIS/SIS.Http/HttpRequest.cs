...
namespace SIS.Http
{
    public class HttpRequest
    {
        public HttpRequest(string httpRequestAsString)
        {
            ...
            var bodyParts = Body.Split(new char[] { '&' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var bodyPart in bodyParts)
            {
                var parameterParts = bodyPart.Split(new char[] { '=' }, 2);
                FormData.Add(HttpUtility.UrlDecode(parameterParts[0]), HttpUtility.UrlDecode(parameterParts[1]));
            }
        }
